...
class NodeService {
    ...
    public static function getActions($className) {
        ...
        if (!empty($actions)) {
            foreach ($actions as $action) {
                if (strpos($action, '_') !== 0 && $action !== 'initialize'  && $action !== 'registerMiddleware'  ) {
                    $actionList[] = $action;
                }
            }
        }
    ...
    public static function refreshNode($modules = []) {
        ...
        foreach ($nodeList as $vo) {
            $nodeInfo = Db::name('system_node')->where(['node' => $vo])->find();
        ...
        if (!empty($insertNode)) {
            Db::startTrans();
            try {
                Db::name('system_node')->insertAll($insertNode);
                Db::commit();
            } catch (\Exception $e) {
                Db::rollback();
    ...
    public static function cleanNode($modules = []) {
        empty($modules) && $modules = self::getFolders(env('app_path'));
        $nodeList = Db::name('system_node')->where(['is_auto' => 0])->select();

        $autoNodeList = self::getNodeList($modules);

        $nodelete = [];
        foreach ($nodeList as $k => $voNode) {
            foreach ($autoNodeList as $voAutoNode) {
                $voNode['node'] == $voAutoNode && $nodelete[$k] = $k;
            }
        ...
        $delete = [];
        foreach ($nodeList as $k => $voNode) {
            !isset($nodelete[$k]) && $delete[] = $voNode['id'];
        }

        if (!empty($delete)) {
            Db::startTrans();
            try {
                Db::name('system_node')->delete($delete);
                Db::commit();
            } catch (\Exception $e) {
                Db::rollback();
                return ['code' => 1, 'msg' => $e->getMessage()];
