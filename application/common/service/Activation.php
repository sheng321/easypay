...
class Activation
{
    ...
    public function sendActiveCode($user){
        ...
        $subject    =   "【聚合支付】 - 开户邮箱激活";
        //邮件主体  也可以使用邮件模板文件
        $content =  self::getRegActiveContent($user,$user['activeUrl']);

        //发送激活邮件
        try{
            return   \app\common\service\Mail::getInstance()->send($toemail,$name,$subject,$content);

        }catch (\Exception $exception){
            __log("Active Code Error:".$exception->getMessage(),'error');
            return false;
        }
    }
    ...
    public function sendRegCallback($user){
        ...
        //邮件主体  也可以使用邮件模板文件
        $content = self::getRegCallbackContent($user,$user['activeUrl']);
        //发送激活邮件

        $config = \app\common\model\SysConfig::getMailConfig();
        try{
           return Mail::getInstance($config)->send($toemail,$name,$subject,$content);
        }catch (\Exception $exception){
    ...
    public function createActiveCode($userInfo){
        ...
        //生成Code
        $code = password(json_encode($data));
        //加密
        //1.用户激活的时候返回 code
        //2.用code解密取到ActiveCode的数据
        //3.比对cache
        //4.完成激活
        $encryptData = (new CryptAes($code))->encrypt(json_encode($userInfo)); //用户信息
     ...
     * @return string
     */
    private function getRegActiveContent($user,$activeUrl){

        //生成code
        $activecode = urlencode($this->createActiveCode($user));
        //激活地址
        $activeUrl = $activeUrl."/{$activecode}";
